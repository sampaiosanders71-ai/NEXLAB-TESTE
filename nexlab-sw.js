...
const GENERATED_AT='2026-09-06T03:44:30Z';
const ASSET_REVISION='app-beta-0-26-82-dashboard-finalizado';
const CACHE_NAME='nexlab-app-beta-0-26-82-dashboard-finalizado-20260906T034430Z';
...
const ALLOWED_TABS=new Set(['dashboard','pendencias','agenda','notificacoes','participantes','permissoes','equipes','perfil','projetos','inventario','patrimonio','estoque','reserva','marketing','eventos','mural','feedback','configuracoes','relatorios','saude-sistema','logs','atividades-sistema']);
...
self.addEventListener('install',event=>{event.waitUntil((async()=>{await stageInstall();await self.skipWaiting();})());});
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const validation=await promoteStaging();await self.clients.claim();const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});for(const client of clients){try{client.postMessage({type:'NEXLAB_SW_ACTIVATED',version:APP_VERSION,release:APP_RELEASE,revision:APP_REVISION,generatedAt:GENERATED_AT,cache:CACHE_NAME,validation,previousCachesRetained:true,emergencyRecovery:true});const target=new URL(client.url);for(const key of ['nexlabActivated','nexlabReload','nexlabModuleRetry','nexlabRecoveryReason','nexlabRecover','nexlabEmergencyReset'])target.searchParams.delete(key);target.searchParams.set('nexlabEmergencyReset',GENERATED_AT.replace(/[^0-9]/g,''));if(typeof client.navigate==='function')await client.navigate(target.href);}catch{}}})());});

(function(){
  ...
  function teamIdentityIcon(state,size=26){const identity=teamIdentity(state);const icon=node('span','nexlab-team-chat-identity-icon-v02682');icon.setAttribute('aria-hidden','true');icon.style.width=`${size}px`;icon.style.height=`${size}px`;icon.style.setProperty('--nexlab-team-chat-icon-mask',`url(./assets/team-icons/${identity.iconKey}.svg?v=app-beta-0-26-82-equipes-cards-reconstruidos)`);icon.style.setProperty('--nexlab-team-chat-accent',identity.color);return icon;}

(function(){
  ...
  style.textContent=`
    #nexlab-validation-trigger{position:fixed;right:18px;bottom:86px;z-index:10045;border:0;border-radius:999px;background:#0b2a63;color:#fff;padding:11px 15px;box-shadow:0 14px 38px rgba(15,23,42,.3);font:800 12px/1.2 system-ui,-apple-system,"Segoe UI",sans-serif;cursor:pointer;display:flex;gap:8px;align-items:center}
    #nexlab-validation-trigger:hover{background:#123a7a}.nexlab-validation-overlay{position:fixed;inset:0;z-index:10110;background:rgba(2,6,23,.72);display:grid;place-items:center;padding:18px}
    ...
    @media(max-width:760px){#nexlab-validation-trigger{right:14px;bottom:78px}.nexlab-validation-overlay{padding:0}.nexlab-validation-dialog{height:100%;max-height:100vh;border-radius:0}.nexlab-validation-head,.nexlab-validation-body{padding-left:16px;padding-right:16px}.nexlab-validation-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.nexlab-validation-card-body{grid-template-columns:1fr}.nexlab-validation-choice{grid-template-columns:1fr}.nexlab-promotion-grid{grid-template-columns:1fr}.nexlab-promotion-grid .full{grid-column:auto}}
  `;
  ...
  async function api(path,options={}){
    ...
    const response=await fetch(`${BASE}${path}`,{...options,headers:{apikey:KEY,Authorization:`Bearer ${token}`,'Content-Type':'application/json',...(options.headers||{})},cache:'no-store'});
    let data=null;try{data=await response.json();}catch{}
    if(!response.ok)throw new Error(data?.message||data?.error_description||data?.hint||`Falha HTTP ${response.status}.`);
    return data;
  }
  ...
  async function currentProfile(force=false){
    if(!force&&cachedProfile&&Date.now()-cachedAt<30000)return cachedProfile;
    const id=jwtSubject(authToken());if(!id)return null;
  ...
  async function openValidation(options={}){
    const {body}=dialog('Validação dos coordenadores','Roteiro oficial de homologação da Beta 0.26.54. Cada item obrigatório precisa de aprovação registrada por Coordenador.');
    const loading=note('Carregando ciclo de validação...');body.appendChild(loading);
    try{
      let state=await rpc('nexlab_get_validation_cycle_v02654');loading.remove();
      ...
      topActions.querySelector('[data-refresh]').onclick=async()=>{state=await rpc('nexlab_get_validation_cycle_v02654');render();};
      ...
      function renderItems(){
        ...
        for(const item of state.items||[]){
          ...
          save.onclick=async()=>{
            ...
            try{state=await rpc('nexlab_submit_validation_review_v02654',{p_item_key:item.item_key,p_verdict:selected,p_notes:textarea.value.trim()||null,p_evidence:{app_version:VERSION,user_agent:navigator.userAgent,viewport:`${innerWidth}x${innerHeight}`}});render();}
      ...
      function renderPromotion(){
        ...
        generate.onclick=async()=>{generate.disabled=true;output.replaceChildren(note('Gerando manifesto de promoção...'));try{const result=await rpc('nexlab_generate_promotion_manifest_v02654',{p_notes:grid.querySelector('[data-notes]').value.trim()||null});const canonical=JSON.stringify(result.manifest);const hash=await sha256(canonical);download(`NEXLAB_PROMOCAO_OFICIAL_${VERSION.replaceAll('.','_')}.json`,JSON.stringify({...result,sha256:hash},null,2));state=await rpc('nexlab_get_validation_cycle_v02654');render();}catch(error){output.replaceChildren(note(error.message||'Falha ao gerar manifesto.','error'));generate.disabled=!s.ready_for_promotion;}};
        const manifest=existing||state.promotion_manifest;if(manifest){
          ...
          confirm.onclick=async()=>{confirm.disabled=true;const out=confirmation.querySelector('[data-confirm-output]');out.replaceChildren(note('Confirmando publicação...'));try{await rpc('nexlab_confirm_official_repository_publish_v02654',{p_manifest_id:manifest.id,p_repository:confirmation.querySelector('[data-repository]').value.trim(),p_commit_sha:confirmation.querySelector('[data-sha]').value.trim(),p_notes:confirmation.querySelector('[data-publication-notes]').value.trim()||null});state=await rpc('nexlab_get_validation_cycle_v02654');render();}catch(error){out.replaceChildren(note(error.message||'Falha ao confirmar publicação.','error'));confirm.disabled=false;}};
        }
      }
      ...
      render();
    }catch(error){loading.className='nexlab-validation-note error';loading.textContent=error.message||'Falha ao abrir a validação.';}
  }

  function openPromotion(){return openValidation({focusPromotion:true});}
  async function mount(){
    if(document.getElementById('nexlab-validation-trigger')||window.__NEXLAB_PROFILE_PREVIEW__?.active)return;
    const profile=await currentProfile();const role=String(profile?.role||'').toLowerCase();if(profile?.ativo===false||role!=='coordenador')return;
    const button=document.createElement('button');button.id='nexlab-validation-trigger';button.type='button';button.textContent='Validar versão';button.title='Validação dos coordenadores';button.onclick=()=>openValidation();document.body.appendChild(button);
  }

  window.NexlabCoordinatorValidation=Object.freeze({version:VERSION,open:openValidation,openValidation,openPromotion,load:()=>rpc('nexlab_get_validation_cycle_v02654')});
  window.addEventListener('nexlab:auth-ready',()=>setTimeout(mount,650));setTimeout(mount,2800);setInterval(()=>{if(!document.getElementById('nexlab-validation-trigger'))mount();},14000);
})();

(function(){
  ...
  const FEEDBACK_ASSIST_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.82';
  ...
  /* Beta 0.26.82 — rótulos de versão, cronologia e metadados alinhados */
...
/* NEXLAB Beta 0.26.82 — Hotfix: aplica referência visual da Agenda ao Marketing. */
;(function(){
  ...
  function initials(value){
    const clean=String(value||'MK').replace(/[^\p{L}\p{N} ]/gu,' ').trim();
    if(!clean) return 'MK';
  ...
  function patchMarketingAgendaReference(){
    if(document.body?.getAttribute('data-nexlab-page')!=='marketing') return;
    const root=document.querySelector('.module-shell.nexlab-marketing-reset') || document.querySelector('.nexlab-marketing-reset');
    const panel=root?.querySelector('.nexlab-mkt-mobile-selected');
    if(!panel) return;
    panel.classList.add('nexlab-mkt-agenda-ref');
    const head=panel.querySelector('.nexlab-mkt-mobile-selected-head');
    const titleWrap=head?.querySelector('div');
    const labelEl=titleWrap?.querySelector('span');
    const titleEl=titleWrap?.querySelector('strong');
    const smallEl=titleWrap?.querySelector('small') || head?.querySelector('small');
    if(head && smallEl && smallEl.parentElement!==head){ head.appendChild(smallEl); }
    const originalLabel = labelEl?.dataset?.nexlabOriginalLabel || labelEl?.textContent || '';
    if(labelEl) labelEl.dataset.nexlabOriginalLabel = originalLabel;
    const countMatch=(smallEl?.textContent||'').match(/\d+/);
    const count=countMatch?Number(countMatch[0]):0;
    if(labelEl && titleEl){
      const currentTitle=(titleEl.textContent||'').replace(/^Dias ocupados em\s+/i,'').trim();
      if(/atividades do dia/i.test(originalLabel)){
        labelEl.textContent='Atividades do dia';
        titleEl.textContent=currentTitle;
      }else{
        labelEl.textContent='Lista de atividades';
        titleEl.textContent=`Dias ocupados em ${currentTitle}`;
      }
    }
    if(smallEl) smallEl.textContent=plural(count,'item','itens');
    const records=panel.querySelectorAll('.nexlab-mkt-mobile-record');
    records.forEach((record)=>{
      const channelEl=record.querySelector('.nexlab-mkt-channel');
      const tone=normalizeTone(channelEl?.textContent||'');
      record.dataset.channelTone=tone;
      record.dataset.short=initials(channelEl?.textContent||'MK');
      const buttons=record.querySelectorAll('.nexlab-mkt-mobile-record-actions button');
      buttons.forEach((btn)=>{
        const txt=String(btn.textContent||'').trim().toLowerCase();
        btn.dataset.action=txt.includes('remov')?'remove':(txt.includes('avanç')||txt.includes('avanc')?'advance':'edit');
      });
    });
  }
  let raf=0;
  function queue(){ cancelAnimationFrame(raf); raf=requestAnimationFrame(patchMarketingAgendaReference); }
  ...
  const start=()=>observer.observe(document.body||document.documentElement,{childList:true,subtree:true,characterData:true});
  if(document.body) start(); else window.addEventListener('DOMContentLoaded', start, {once:true});
})();

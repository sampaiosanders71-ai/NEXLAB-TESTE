(function(root){
  ...
  const REVISION='beta-0-26-74-inventario-rolagem-formularios';
  const RPC='nexlab_get_release_publish_gate_v02674';
  const ASSERT_RPC='nexlab_assert_release_publishable_v02674';
  ...
  async function assertPromotable(){const client=await waitClient();if(!client)throw new Error('Cliente Supabase indisponível.');const result=await client.rpc(ASSERT_RPC);if(result?.error)throw result.error;return result.data;}

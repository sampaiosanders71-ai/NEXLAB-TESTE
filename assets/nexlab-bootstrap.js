(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.81',release:'Beta',revision:'beta-0-26-81-minimizacao-dados-pessoais',assetRevision:'app-beta-0-26-81-minimizacao-dados-pessoais',cacheName:'nexlab-beta-0-26-81-minimizacao-dados-pessoais',generatedAt:'2026-08-21T21:03:00Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.81';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.81';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.81';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.81';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.81';
  ...
  window.addEventListener('nexlab:dashboard-bundle-metrics',(event)=>{
    ...
    try{sessionStorage.setItem('nexlab:dashboard-bundle:v0.26.81',JSON.stringify(detail));}catch{}
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.81', JSON.stringify(performanceState));
    } catch {}
...
/* NEXLAB Beta 0.26.81 — recursos pós-abertura com nova tentativa controlada. */
(function(){
  'use strict';
  const BUILD=globalThis.__NEXLAB_BUILD_IDENTITY__||{};
  const VERSION=BUILD.version||'0.26.81';
  const REVISION=BUILD.revision||'beta-0-26-81-minimizacao-dados-pessoais';
  ...
  ]).map(path=>'./'+String(path).replace(/^\.\//,'')+'?v='+(BUILD.assetRevision||'app-beta-0-26-81-minimizacao-dados-pessoais'));

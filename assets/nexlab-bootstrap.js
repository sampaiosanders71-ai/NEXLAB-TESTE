(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.47',release:'Beta',revision:'beta-0-26-47-projetos-resumo-compacto',assetRevision:'app-beta-0-26-47-projetos-resumo-compacto',cacheName:'nexlab-beta-0-26-47-projetos-resumo-compacto',generatedAt:'2026-07-30T01:25:00Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.47';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.47';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.47';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.47';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.47';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.47', JSON.stringify(performanceState));
    } catch {}
...
/* NEXLAB Beta 0.26.47 — recursos pós-abertura com nova tentativa controlada. */
(function(){
  'use strict';
  const BUILD=globalThis.__NEXLAB_BUILD_IDENTITY__||{};
  const VERSION=BUILD.version||'0.26.47';
  const REVISION=BUILD.revision||'beta-0-26-47-projetos-resumo-compacto';
  ...
  ]).map(path=>'./'+String(path).replace(/^\.\//,'')+'?v='+(BUILD.assetRevision||'app-beta-0-26-47-projetos-resumo-compacto'));

(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.74',release:'Beta',revision:'beta-0-26-74-inventario-rolagem-formularios',assetRevision:'app-beta-0-26-74-inventario-rolagem-formularios',cacheName:'nexlab-beta-0-26-74-inventario-rolagem-formularios',generatedAt:'2026-08-06T00:12:53Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.74';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.74';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.74';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.74';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.74';
  ...
  window.addEventListener('nexlab:dashboard-bundle-metrics',(event)=>{
    ...
    try{sessionStorage.setItem('nexlab:dashboard-bundle:v0.26.74',JSON.stringify(detail));}catch{}
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.74', JSON.stringify(performanceState));
    } catch {}
...
/* NEXLAB Beta 0.26.74 — recursos pós-abertura com nova tentativa controlada. */
(function(){
  'use strict';
  const BUILD=globalThis.__NEXLAB_BUILD_IDENTITY__||{};
  const VERSION=BUILD.version||'0.26.74';
  const REVISION=BUILD.revision||'beta-0-26-74-inventario-rolagem-formularios';
  ...
  ]).map(path=>'./'+String(path).replace(/^\.\//,'')+'?v='+(BUILD.assetRevision||'app-beta-0-26-74-inventario-rolagem-formularios'));

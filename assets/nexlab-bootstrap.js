(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.79',release:'Beta',revision:'beta-0-26-79-consistencia-seguranca',assetRevision:'app-beta-0-26-79-consistencia-seguranca',cacheName:'nexlab-beta-0-26-79-consistencia-seguranca',generatedAt:'2026-08-06T00:12:53Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.79';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.79';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.79';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.79';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.79';
  ...
  window.addEventListener('nexlab:dashboard-bundle-metrics',(event)=>{
    ...
    try{sessionStorage.setItem('nexlab:dashboard-bundle:v0.26.79',JSON.stringify(detail));}catch{}
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.79', JSON.stringify(performanceState));
    } catch {}
...
/* NEXLAB Beta 0.26.79 — recursos pós-abertura com nova tentativa controlada. */
(function(){
  'use strict';
  const BUILD=globalThis.__NEXLAB_BUILD_IDENTITY__||{};
  const VERSION=BUILD.version||'0.26.79';
  const REVISION=BUILD.revision||'beta-0-26-79-consistencia-seguranca';
  ...
  ]).map(path=>'./'+String(path).replace(/^\.\//,'')+'?v='+(BUILD.assetRevision||'app-beta-0-26-79-consistencia-seguranca'));

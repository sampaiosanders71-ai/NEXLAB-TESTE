(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.45',release:'Beta',revision:'beta-0-26-45-cabecalhos-escuros-modulos',assetRevision:'app-beta-0-26-45-cabecalhos-escuros-modulos',cacheName:'nexlab-beta-0-26-45-cabecalhos-escuros-modulos',generatedAt:'2026-07-30T00:04:01Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.45';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.45';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.45';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.45';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.45';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.45', JSON.stringify(performanceState));
    } catch {}
...
/* NEXLAB Beta 0.26.45 — recursos pós-abertura com nova tentativa controlada. */
(function(){
  'use strict';
  const BUILD=globalThis.__NEXLAB_BUILD_IDENTITY__||{};
  const VERSION=BUILD.version||'0.26.45';
  const REVISION=BUILD.revision||'beta-0-26-45-cabecalhos-escuros-modulos';
  ...
  ]).map(path=>'./'+String(path).replace(/^\.\//,'')+'?v='+(BUILD.assetRevision||'app-beta-0-26-45-cabecalhos-escuros-modulos'));

(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.39',release:'Beta',revision:'beta-0-26-39-version-labels-chronology-rpc-governance',assetRevision:'app-beta-0-26-39-version-labels-chronology-rpc-governance',cacheName:'nexlab-beta-0-26-39-version-labels-chronology-rpc-governance',generatedAt:'2026-07-29T19:46:00Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.39';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.39';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.39';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.39';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.39';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.39', JSON.stringify(performanceState));
    } catch {}
...
/* NEXLAB Beta 0.26.39 — recursos pós-abertura com nova tentativa controlada. */
(function(){
  'use strict';
  const BUILD=globalThis.__NEXLAB_BUILD_IDENTITY__||{};
  const VERSION=BUILD.version||'0.26.39';
  const REVISION=BUILD.revision||'beta-0-26-39-version-labels-chronology-rpc-governance';
  ...
  ]).map(path=>'./'+String(path).replace(/^\.\//,'')+'?v='+(BUILD.assetRevision||'app-beta-0-26-39-version-labels-chronology-rpc-governance'));

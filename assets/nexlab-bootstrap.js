(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.41',release:'Beta',revision:'beta-0-26-41-marketing-single-create-action',assetRevision:'app-beta-0-26-41-marketing-single-create-action',cacheName:'nexlab-beta-0-26-41-marketing-single-create-action',generatedAt:'2026-07-29T21:15:00Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.41';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.41';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.41';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.41';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.41';
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.41', JSON.stringify(performanceState));
    } catch {}
...
/* NEXLAB Beta 0.26.41 — recursos pós-abertura com nova tentativa controlada. */
(function(){
  'use strict';
  const BUILD=globalThis.__NEXLAB_BUILD_IDENTITY__||{};
  const VERSION=BUILD.version||'0.26.41';
  const REVISION=BUILD.revision||'beta-0-26-41-marketing-single-create-action';
  ...
  ]).map(path=>'./'+String(path).replace(/^\.\//,'')+'?v='+(BUILD.assetRevision||'app-beta-0-26-41-marketing-single-create-action'));

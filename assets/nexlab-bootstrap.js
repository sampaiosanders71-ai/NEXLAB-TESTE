(function(){
  ...
  const BUILD_IDENTITY = window.__NEXLAB_BUILD_IDENTITY__ || Object.freeze({version:'0.26.73',release:'Beta',revision:'beta-0-26-73-gate-homologacao-fisica',assetRevision:'app-beta-0-26-73-gate-homologacao-fisica',cacheName:'nexlab-beta-0-26-73-gate-homologacao-fisica',generatedAt:'2026-08-06T00:12:53Z'});
  ...
  const OBSERVABILITY_QUEUE_KEY = 'nexlab:observability:queue:v0.26.73';
  const OBSERVABILITY_DEDUP_KEY = 'nexlab:observability:dedup:v0.26.73';
  ...
  const USER_ERROR_CONTEXT_KEY = 'nexlab:feedback-assist:context:v0.26.73';
  const USER_ERROR_STATE_KEY = 'nexlab:user-error-state:v0.26.73';
  ...
  const PERFORMANCE_ALERT_STATE_KEY = 'nexlab:performance-alert-state:v0.26.73';
  ...
  window.addEventListener('nexlab:dashboard-bundle-metrics',(event)=>{
    ...
    try{sessionStorage.setItem('nexlab:dashboard-bundle:v0.26.73',JSON.stringify(detail));}catch{}
  ...
  function savePerformanceState(){
    ...
    try {
      sessionStorage.setItem('nexlab:performance:v0.26.73', JSON.stringify(performanceState));
    } catch {}
...
/* NEXLAB Beta 0.26.73 — recursos pós-abertura com nova tentativa controlada. */
(function(){
  'use strict';
  const BUILD=globalThis.__NEXLAB_BUILD_IDENTITY__||{};
  const VERSION=BUILD.version||'0.26.73';
  const REVISION=BUILD.revision||'beta-0-26-73-gate-homologacao-fisica';
  ...
  ]).map(path=>'./'+String(path).replace(/^\.\//,'')+'?v='+(BUILD.assetRevision||'app-beta-0-26-73-gate-homologacao-fisica'));

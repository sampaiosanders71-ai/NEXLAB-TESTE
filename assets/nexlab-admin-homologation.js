(function(){
  ...
  async function openSecurityExport(){
    ...
    const purpose=document.createElement('label');purpose.className='nexlab-admin-field';purpose.innerHTML='<span>Finalidade da exportação</span><textarea>Homologação física da Beta 0.26.73 e conferência do gate de promoção oficial.</textarea>';body.appendChild(purpose);
  ...
  async function openReleaseReview(){
    ...
    try{
      ...
      const results=await Promise.allSettled([
        ...
        rpc('nexlab_get_homologation_diagnostics_v02673'),
        loadPreviewData()
      ]);
